...
# 统一的 Python 命令（check_python_version 会设置）
PYTHON_CMD=""
...
print_banner() {
    print_divider
    echo -e "  ${BOLD}${CYAN}wwwOK${NC} ${WHITE}Proxy Panel v2.1${NC}"
...
#-----------------------------------------------------------
# Python 版本检查和自动安装
# 要求: Python 3.6+ (wwwOK_api.py 需要)
#-----------------------------------------------------------
check_python_version() {
    echo -e "\n${CYAN}>>> 检查 Python 版本...${NC}"

    local py_ver="" py_cmd=""

    # 1. 尝试找到已安装的 Python 3.6+
    for cmd in python3 python3.11 python3.10 python3.9 python3.8 python3.7 python3.6 python; do
        if command -v $cmd &>/dev/null; then
            py_ver=$($cmd -c 'import sys; print("%d.%d" % (sys.version_info.major, sys.version_info.minor))' 2>/dev/null)
            if [ -n "$py_ver" ]; then
                local major=$(echo $py_ver | cut -d. -f1)
                local minor=$(echo $py_ver | cut -d. -f2)
                if [ "$major" -eq 3 ] && [ "$minor" -ge 6 ]; then
                    py_cmd="$cmd"
                    echo -e "  ${GREEN}找到 Python: ${py_cmd} (v${py_ver})${NC}"
                    break
                fi
            fi
        fi
    ...
    # 2. 未找到符合要求的 Python，自动安装
    if [ -z "$py_cmd" ]; then
        echo -e "  ${YELLOW}未找到 Python 3.6+，正在自动安装...${NC}"

        if command -v apt-get &>/dev/null; then
            # Debian/Ubuntu
            echo -e "  ${CYAN}使用 apt 安装 Python 3...${NC}"
            apt-get update -qq
            apt-get install -y -qq python3 python3-pip python3-dateutil

        elif command -v dnf &>/dev/null; then
            # Fedora / AlmaLinux / Rocky
            echo -e "  ${CYAN}使用 dnf 安装 Python 3...${NC}"
            dnf install -y python3 python3-pip python3-dateutil

        elif command -v yum &>/dev/null; then
            # CentOS 7 / 旧版
            echo -e "  ${CYAN}使用 yum 安装 Python 3...${NC}"
            if yum info python3 &>/dev/null; then
                yum install -y python3 python3-pip python3-dateutil
            else
                # CentOS 7 没有 python3，尝试 IUS 仓库
                echo -e "  ${YELLOW}系统 Python 3 不存在，尝试安装 IUS 仓库...${NC}"
                yum install -y epel-release
                if command -v python36 &>/dev/null; then
                    py_cmd="python36"
                    py_ver=$($py_cmd -c 'import sys; print("%d.%d" % (sys.version_info.major, sys.version_info.minor))' 2>/dev/null)
                fi
                if [ -z "$py_cmd" ] || [ "$py_ver" != "3.6" ]; then
                    # 用 python3-rhel-mode 或直接提示
                    echo -e "  ${CYAN}安装 IUS Python 3.11...${NC}"
                    yum install -y https://repo.ius.io/ius-release-el7.rpm 2>/dev/null || true
                    yum install -y python3 python3-pip python3-dateutil 2>/dev/null || true
                fi
            fi

        elif command -v apk &>/dev/null; then
            # Alpine
            echo -e "  ${CYAN}使用 apk 安装 Python 3...${NC}"
            apk add --no-cache python3 py3-pip py3-dateutil

        else
            echo -e "${RED}无法自动安装 Python，请手动安装 Python 3.6+ 后重试${NC}"
            echo -e "${YELLOW}Ubuntu/Debian: apt install python3 python3-pip python3-dateutil${NC}"
            echo -e "${YELLOW}CentOS/RHEL:   yum install python3 python3-pip python3-dateutil${NC}"
            exit 1
        fi
    ...
    # 3. 最终验证
    if [ -z "$py_cmd" ]; then
        echo -e "${RED}Python 3.6+ 检查失败${NC}"
        exit 1
    fi

    PYTHON_CMD="$py_cmd"
    echo -e "  ${GREEN}Python 版本检查通过: ${PYTHON_CMD} (v${py_ver})${NC}"
}
...
install_dependencies() {
    ...
    # 检查 dateutil（PYTHON_CMD 已确定）
    if ! $PYTHON_CMD -c "import dateutil" 2>/dev/null; then
        echo -e "${CYAN}  安装 python3-dateutil...${NC}"
        $PYTHON_CMD -m pip install python-dateutil -q 2>/dev/null || \
        pip3 install python-dateutil -q 2>/dev/null || \
        apt install -y python3-dateutil -qq 2>/dev/null || \
        yum install -y python3-dateutil 2>/dev/null || true
    fi
...
install_api_service() {
    ...
    # 查找 python3 实际路径
    local PYTHON_BIN=$(command -v $PYTHON_CMD 2>/dev/null || echo "/usr/bin/python3")
...
[Service]
Type=simple
ExecStart=${PYTHON_BIN} /opt/wwwOK/scripts/wwwOK_api.py $API_PORT
Restart=always
RestartSec=5
StandardOutput=journal
StandardError=journal
